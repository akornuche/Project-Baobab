...
async function main() {
  ...
  // Create Subdomains for Government
  const governmentSubdomains = [
  ...
  for (const subdomain of governmentSubdomains) {
  ...
  for (const source of sources) {
    try {
      const created = await prisma.guideSource.create({
        data: {
          guideId: guide.id,
          url: source.url,
          title: source.title,
      ...
      // Skip if already exists
      console.log(`⊘ Source already exists: ${source.title}`);
    }
  }

  console.log('\n✅ Database seeding completed successfully!');
  console.log(`   - 3 domains created`);
  console.log(`   - 12 subdomains created (4 per domain)`);
  console.log(`   - 3 users created`);
  console.log(`   - 1 sample guide created`);
  console.log(`   - 1 sample tool created`);
  console.log(`   - 2 directory listings created`);
  console.log(`   - Official sources created`);
}
